            ...
            <div class="card">
                <div class="card-header">
                    <div class="col-md-12 text-right">
                        <a href=" {{ route('Retur.Add') }} " class="btn btn-info"><i class='fas fa-plus'> Tambah
                                Data
                            </i></a>
                    </div>
                </div>
                        ...
                        <form method="POST" action="javascript:void(0)" id="formSearchRetur">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="id_retur" id="id_retur" class="form-control">
...
@push('after-script')
    <script>
        ...
        function resetModal() {
            $('#detailListRetur').html("");
        }

        function getDetail(idRetur) {
            $("#exampleModal").modal("show");
            $.ajax({
                type: "POST",
                url: "{{ Route('Retur.Detail') }}",
                data: {
                    _token: '{{ csrf_token() }}',
                    id: idRetur
                },
                dataType: "HTML",
                beforeSend: function() {
                    $('#loaderMdl').fadeIn('slow');
                },
                success: function(response) {
                    $('#loaderMdl').fadeOut('slow');
                    $('#detailListRetur').html(response);
                }
            });
        }
...
@endpush
@section('modal')
    @include('Transaction.Retur.modalDetailRetur')
@endsection
